...
impl FeedForward {
    ...
    pub fn enable_error_logging(&mut self, log_file: String, epoch_interval: i32) {
        ...
        for j in (0..self.layers.len()).rev() {
            ...
            if j == self.layers.len() - 1 {
                ...
                self.recent_average_error =
                    (self.recent_average_error * 100.0 + self.error) / (100.0 + 1.0);
                if !self.log_file.is_empty() && self.epoch % self.epoch_interval == 0 {
    ...
    /// nn.train(&d, 30_000);
    /// ```
    pub fn train<T>(&mut self, data: &T)
    where
        T: Extractable,
    {
        for _ in 0..data.len() {
    ...
    pub fn train_mem_efficient(&mut self, file_path: &str) {
    ...
    pub fn activation(&mut self, func: activators::Type) -> &mut FeedForward {
        match func {
            ...
            activators::Type::ELU => {
                self.act_type = activators::Type::ELU;
                self.act.func = activators::elu;
                self.act.der = activators::der_elu;
            }
        }
        self
    }
